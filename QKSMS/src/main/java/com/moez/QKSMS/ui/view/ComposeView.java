...
public class ComposeView extends LinearLayout implements View.OnClickListener {
    ...
    StringBuilder getBL,getNum;

    //blacklist_content_provider
    String AUTHORITY = "com.plamera.tmswiftlauncher.Provider.BlackListProvider";
    Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY + "/blacklistno");
    String KEY_BLACK_LIST = "blacklist_number";
    ...
    public void sendSms() {
        getBL = new StringBuilder();
        Uri uri = Uri.parse(String.valueOf(CONTENT_URI));
        Cursor c = mContext.managedQuery(uri, null, null, null, "id");
        if (c.moveToFirst()) {
            do{
                String getBlack = c.getString(c.getColumnIndex(KEY_BLACK_LIST));
                getBL.append(getBlack);
            } while (c.moveToNext());
        }
        String blackList = getBL.toString();
